...
from spider_xhs.apis.xhs_creator_login_apis import XHSCreatorLoginApi
from spider_xhs.utils.common_util import init
from spider_xhs.utils.data_util import handle_note_info, download_note, save_to_xlsx, ocr_note_images
from spider_xhs.utils.ocr_util import build_ocr_client_from_env
from spider_xhs.utils.xhs_util import get_common_headers
...
def sanitize_excel_name(name: str):
    if not name:
        return "export"
    value = re.sub(r"|[\\/:*?\"<>| ]+", "", str(name)).replace('\n', '').replace('\r', '')
...
class Data_Spider():
    ...
    def _build_user_excel_name(self, user_url: str, cookies_str: str, proxies=None):
        url_parse = urllib.parse.urlparse(user_url)
        user_id = url_parse.path.rstrip('/').split('/')[-1].split('?')[0]
        nickname = ""
        try:
            ok, _msg, res_json = self.xhs_apis.get_user_info(user_id, cookies_str, proxies)
            if ok:
                user_data = (res_json or {}).get('data') or {}
                basic_info = user_data.get('basic_info') or {}
                nickname = (basic_info.get('nickname') or "").strip()
        except Exception:
            pass
        if nickname:
            return sanitize_excel_name(f"{nickname}_{user_id}")
        return sanitize_excel_name(user_id)
    ...
    def spider_user_all_note(self, user_url: str, cookies_str: str, base_path: dict, save_choice: str, excel_name: str = '', proxies=None):
        ...
        try:
            ...
            if save_choice == 'all' or save_choice == 'excel':
                excel_name = self._build_user_excel_name(user_url, cookies_str, proxies)
...
def parse_args():
    ...
    parser.add_argument('--ocr-submit-retry-delay', type=int, default=None, help='OCR 提交队列满时重试间隔秒（默认读取环境变量或 30）')
...
def main():
    ...
    # save_choice: all=全部信息, media=视频和图片(media-video只视频/media-image只图片), excel=仅excel。
    ...
    if mode not in {"exit", "ocr"} and not (cookies_str or '').strip():
        logger.error(
            '未检测到环境变量 COOKIES。请在项目根目录的 .env 文件中配置 COOKIES=（从浏览器复制完整 Cookie）。'
        )
        raise SystemExit(1)
